...
$parentModule = 'AzureDevOps.Services.OpenApi'
if (-not $ModuleName) { $ModuleName = 'AzureDevOps.Services.OpenApi.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\AzureDevOps.Services.OpenApi'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\AzureDevOps.Services.OpenApi.psd1"
$prereqHash = @{
